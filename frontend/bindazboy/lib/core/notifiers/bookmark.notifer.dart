...
class BookmarkNotifier extends ChangeNotifier {
  final BookmarkAPI _bookmarkApi = new BookmarkAPI();
  ...
  Future addBookmark({
    ...
    try {
      var data = await utilityNotifier(context: context)
          .readUserEmail(context: context);
      var response = await _bookmarkApi.addBookmark(
          useremail: data.toString(), blog_id: blog_id, context: context);
  ...
  Future fetchBookmarksByUser({required BuildContext context}) async {
    try {
      ...
      var response = await _bookmarkApi.fetchBookmarksByUser(
          context: context, useremail: data.toString());
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Somthing went wrong,try again"),
        ),
      );
    }
  }
  ...
  Future deleteBookmark(
      {required BuildContext context, required dynamic bookmark_id}) async {
    try {
      var response = await _bookmarkApi.deleteBookmark(
          bookmark_id: bookmark_id, context: context);
      final Map<String, dynamic> parsedData = await response;
      final bookmarkStatusCode = parsedData["code"];
  ...
  Stream checkbookmark({
    ...
    required dynamic blog_id,
      ...
      Stream.periodic(Duration(seconds: 2))
          .asyncMap((_) => checkIfBookmarkExists(
                context: context,
                blog_id: blog_id,
              ));
  ...
  Future<bool> checkIfBookmarkExists({
    ...
    var data =
        await utilityNotifier(context: context).readUserEmail(context: context);
