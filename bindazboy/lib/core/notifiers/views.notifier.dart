...
class ViewsNotifier extends ChangeNotifier {
  ...
  Future addView({
    ...
    try {
      ...
      var viewdata0 = await viewApi.addpostViews(
        context: context,
        useremail: _loggedgmail1,
        post_id: fspostid,
      );

      final Map<String, dynamic> parsedData = await json.decode(viewdata0);
  ...
  Future featchViews({
    ...
    try {
      var likedata = await viewApi.featchpostViews(
        context: context,
        blog_id: post_id,
      );
      final Map<String, dynamic> parsedData = await json.decode(likedata);
  ...
  Future checkViews({
    ...
    try {
      ...
      var likedata = await viewApi.checkpostViews(
        context: context,
        blog_id: post_id,
        useremail: _loggedgmail1,
      );
