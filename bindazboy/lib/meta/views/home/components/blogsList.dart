...
class Bloglist extends StatefulWidget {
  dynamic snapshot;
  bool? detailpage;
  Bloglist({super.key, this.snapshot});
    ...
    if (allloaded) {
      ...
      await Future.delayed(const Duration(milliseconds: 500)).whenComplete(
        () async {
          ...
          List<String> newData =
              items.length >= widget.snapshot.length
                  ? []
      ...
      child: SliverList(
        ...
        delegate: SliverChildBuilderDelegate(
          ...
          (context, index) {
            ...
            final blog = widget.snapshot[index] as Data;
            return GestureDetector(
              onTap: () async {
                ...
                await viewprovider
                    .checkViews(context: context, post_id: blog.blogId)
                    .whenComplete(() async {
                      if (viewprovider.isviewData == false) {
                        //  EasyLoading.dismiss();
                        await viewprovider
                            .addView(context: context, fspostid: blog.blogId)
                            .then((v) {
                              viewprovider
                                  .featchViews(
                                    context: context,
